...
export default function Interactions() {
  const [defaultImageUrl, setdefaultImageUrl] = useState(
    "https://d3wqbogi93pb3.cloudfront.net/images/homepage_metaData.png"
  );
  const { id } = useParams();
  ...
  const getInteraction = async (id) => {
    const response = await fetch(
      "https://aiverse-backend.vercel.app/api/getAll",
      {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    if (video?.images[0]) {
      setdefaultImageUrl(
        "https://d3wqbogi93pb3.cloudfront.net/" + video.images[0]
      );
    }
    ...
    <div className="container-fluid" style={{ textAlign: "left" }}>
      {interactions && video && (
        <div className="row p-4">
          <Helmet>
            <title>
              {video.name} - {video.pattern} AI-UX Interaction
            </title>
            <meta name="description" content={video.content} />
            {/* <!-- Open Graph / Facebook --> */}
            <meta
              property="og:title"
              content={`${video.name} - ${video.pattern} AI-UX Interaction`}
            />
            ...
            <div className="row">
              {moreAppname && (
                <div className="col-12 mt-5">
                  <h3 className="fonts">
                    More{" "}
                    <span style={{ fontWeight: 700 }}>{video.appName} </span>{" "}
                    pattern
                  </h3>
                </div>
              )}
                ...
                moreAppname.map((item, index) => {
                  return (
                    <div
                      ...
                      onClick={() => {
                        navigate(`../Browse/${item.appName}-${item._id}`, {
                          state: item,
                        });
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <span>
                            <img
                              src={`${cdnURL}${item.iconUrl}`}
                              style={{ margin: "2px", marginRight: "7px" }}
                              width={15}
                              height={15}
                            />
                          </span>
            ...
            <div className="row">
              {morePattern && (
                <div className="col-12 mt-5 fonts">
                  <h3 className="fonts">
                    More{" "}
                    <span style={{ fontWeight: 700 }}>{video.pattern} </span>
                    AI-UX Interactions
                  </h3>
                </div>
              )}
